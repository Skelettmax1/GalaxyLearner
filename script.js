class FlashcardManager {
    ...
    initializeEventListeners() {
        ...
        const handleTouchEnd = (e) => {
            ...
            // Adjust timing for iPad
            const isIPad = /iPad|MacIntel/.test(navigator.platform) || 
                          (navigator.maxTouchPoints && navigator.maxTouchPoints > 1);
            const maxTapDuration = isIPad ? 400 : 300;
            const tapThreshold = isIPad ? 40 : 30;

            if (touchDuration < maxTapDuration && moveX < tapThreshold && moveY < tapThreshold) {
                e.preventDefault();
                flashcard.classList.toggle('flipped');
                return;
            }

            // Calculate swipe with iPad-specific threshold
            const velocity = moveX / touchDuration;
            const swipeThreshold = window.innerWidth * (isIPad ? 0.1 : 0.15);

            if (moveX > swipeThreshold || velocity > (isIPad ? 0.4 : 0.5)) {
        ...
        flashcard.addEventListener('touchend', (e) => {
            if (tapTimeout) {
                ...
                // If it was a simple tap without much movement
